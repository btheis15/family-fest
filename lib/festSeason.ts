/**
 * The Family Fest "season" model — the shared spine that lets the Family Fest
 * app and the resort app (MLR) behave as ONE experience that rises and recedes
 * across the year instead of two separate apps. There's no shared backend yet,
 * so this module is mirrored byte-for-byte in both repos (the same way the
 * EVENT / FAMILY_FEST seed data is mirrored) and reads each repo's own event
 * dates.
 *
 * Compute it on the CLIENT (it depends on "now") via `useFestSeason` so the
 * live-week behavior is correct on the static GitHub Pages build as well as on
